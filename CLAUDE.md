...
- **Framework**: TanStack Start (React 19 + TanStack Router, file-based routes) on Vite, built with Nitro's `cloudflare-module` preset. Deployed as a plain Cloudflare **Worker** (not Pages — there is no Pages project for this repo) via manual `npx wrangler deploy --config .output/server/wrangler.json --name maherkhatib`; there's no git-connected CI/CD, so a push to `main` does not deploy. See `DEPLOYMENT.md` for the full deploy/troubleshooting runbook (required `--name` override, the `fix-assets.js` script that must NOT be run, etc.). `vercel.json` configures an alternate Vercel target (`npm install --legacy-peer-deps`, output `.output/public`).
